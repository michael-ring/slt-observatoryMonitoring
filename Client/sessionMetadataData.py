      ...
      if fitsHeader.get('IMAGETYP') == 'LIGHT':
        ...
        if key not in tempresult:
          tempresult[key]={
                     ...
                     'ra':  int(fitsHeader['OBJCTRA'].split()[0])+(int(fitsHeader['OBJCTRA'].split()[1])*60+int(fitsHeader['OBJCTRA'].split()[2]))/3600,
                     'dec': int(fitsHeader['OBJCTDEC'].split()[0])+(int(fitsHeader['OBJCTDEC'].split()[1])*60+int(fitsHeader['OBJCTDEC'].split()[2]))/3600,
