...
def sky_object_plot(objectra,objectdec, location, obs_datetime=None):
  # identify location in geocentric coordinates
  observation_location = EarthLocation.from_geodetic(location['longitude'], location['latitude'],location['elevation'])
  ...
  sky_object = SkyCoord([f"{objectra} {objectdec}"], frame="icrs", unit=(u.hourangle, u.deg))
  ...
  for i in range(len(sun_alt)):
    if sunset == None and sun_alt[i].value <0 :
      sunset=i
    if endcivil == None and sun_alt[i].value <-6 :
      endcivil=i
    if endnautical == None and sun_alt[i].value <-12 :
      endnautical=i
    if endastronomical == None and sun_alt[i].value <-18 :
      endastronomical=i

  for i in reversed(range(len(sun_alt))):
    if sunrise == None and sun_alt[i].value <0 :
      sunrise=i
    if begincivil == None and sun_alt[i].value <-6 :
      begincivil=i
    if beginnautical == None and sun_alt[i].value <-12 :
      beginnautical=i
    if beginastronomical == None and sun_alt[i].value <-18 :
      beginastronomical=i
