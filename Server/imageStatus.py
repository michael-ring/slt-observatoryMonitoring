        ...
        """
          with tag('tr'):
            ...
            if hasAllSkyImages:
              with tag('td'):
                if runningOnServer():
                  doc.attr(('class', 'allsky'), ('data-src', f'https://{rootserver['name']}/images/{telescopes[telescopeName]['location']}/{Path(allSkyImage).name}'))
                  doc.stag('img', src=f'https://{rootserver['name']}/images/{telescopes[telescopeName]['location']}/{Path(allSkyImageThumb).name}',height="50px")
                else:
